                ...
                <div class="flex-1">
                    <form method="GET" action="{{ route('admin.orders.index') }}" class="relative">
                        <div class="relative">
                            ...
                            <button type="submit" class="absolute right-3 top-2.5 text-gray-400 hover:text-blue-500 transition duration-200">
        ...
        <div class="lg:hidden space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="px-2 py-1 rounded-full text-xs font-medium {{ $order->status == 'Pending' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                ...
                <div class="p-4">
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <a href="{{ asset('storage/bukti_pembayaran/' . $order->payment->bukti) }}"
                                target="_blank"
                ...
                <div class="px-4 pb-4 pt-3 border-t border-gray-100">
                    ...
                    @else
                    <button class="w-full bg-gray-200 text-gray-700 font-medium py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 cursor-not-allowed" disabled>
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        Sudah Dibayar
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="py-4 px-6 text-left text-gray-700 font-semibold">Order #</th>
                            <th class="py-4 px-6 text-left text-gray-700 font-semibold">Customer</th>
                            <th class="py-4 px-6 text-left text-gray-700 font-semibold">Total</th>
                            <th class="py-4 px-6 text-left text-gray-700 font-semibold">Status</th>
                            <th class="py-4 px-6 text-left text-gray-700 font-semibold">Metode</th>
                            <th class="py-4 px-6 text-left text-gray-700 font-semibold">Bayar</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="py-4 px-6">
                                @if($order->status == 'Pending')
                                <span class="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm font-medium flex items-center gap-1 w-fit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    Belum Dibayar
                                ...
                                @else
                                <span class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium flex items-center gap-1 w-fit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    Sudah Dibayar
                            ...
                            <!-- Payment Proof -->
                            <td class="py-4 px-6">
                                @if($order->payment && $order->payment->bukti)
                                <a href="{{ asset('storage/' . $order->payment->bukti) }}"
                                    target="_blank"
                            ...
                            <td class="py-4 px-6">
                                ...
                                @else
                                <button class="inline-flex items-center gap-2 px-4 py-2 bg-gray-200 text-gray-600 rounded-lg font-medium text-sm cursor-not-allowed" disabled>
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    Sudah Lunas
                ...
                <div>
                    ...
                    <ul class="text-gray-600 space-y-1">
                        <li class="flex items-start gap-2">
                            ...
                            <span>Pastikan customer telah mengirimkan bukti pembayaran sebelum menandai sebagai "Sudah Bayar"</span>
                        </li>
                        <li class="flex items-start gap-2">
                            <svg class="w-5 h-5 text-blue-500 mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                        ...
                        <li class="flex items-start gap-2">
                            ...
                            <span>Setelah dikonfirmasi, sistem akan mengubah status order menjadi "processing" secara otomatis</span>
                        </li>
                    </ul>
                </div>
            </div>
...
<div id="confirmModal" class="hidden fixed inset-0 bg-transparent bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-linear-to-br from-black to-red-500 border border-yellow-500 rounded-xl p-6 w-full max-w-md shadow-lg">
        ...
        <div class="flex justify-end space-x-3">
            <button type="button" onclick="closeModal()" class="px-4 py-2 border border-gray-300 text-gray-100 rounded-lg hover:bg-red-500 transition duration-200">
                Batal
            </button>
            <button id="confirmButton" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition duration-200">
                Ya, Konfirmasi
            </button>
        </div>
    </div>
